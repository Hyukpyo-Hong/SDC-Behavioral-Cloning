...
def telemetry(sid, data):
    ...
    steering_angle = float(model.predict_generator(img_gen.flow(
        transformed_image_array, batch_size=1), val_samples=1))
    ...
    if (abs(steering_angle) < 0.1):  # 2.5degree
        ...
        if(speed < 7):
            ...
            throttle = 0.05
    print("Angle: ", format(steering_angle * 25, '.2f'), "(",
          format(steering_angle, '.3f'), ")", " Throttle: ", throttle, " Speed: ", speed)
